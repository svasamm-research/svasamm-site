...
def main() -> int:
    ...
    # Derive the slug list from the .webp files actually in public/hero/ (not just this
    # run's inputs) so processing a single new image never drops the other heroes.
    disk_slugs = sorted(p.stem for p in out_dir.glob("*.webp"))
    manifest.write_text(
    ...
    print(f"manifest: {manifest.relative_to(manifest.parent.parent)} ({len(disk_slugs)} slugs)")
    if missing:
